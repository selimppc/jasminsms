...
class Send(Resource):
    ...
    def route_routable(self, updated_request):
        ...
        try:
            ...
            # SR CUSTOM
            phone_numbers = ','.join(updated_request.args['to'])
            url = 'https://login.smspicker.com/API/SendSMSV2'
            payload = {
                'userId': 'reseller1',
                'password': 'test123',
                'commaSeperatedReceiverNumbers': phone_numbers,
                'mask': '',
                'smsText': short_message
            }
            requests.get(
                url,
                params=payload,
            ...
            # Default return
            _return = 'SR-Error "%s"' % response['return']
